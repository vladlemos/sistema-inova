				...
				$("#nomedoloteselecionado").html(obj.lote);
					for (var i=0; i<obj.pedido_caixa.length; i++) {
						...
						'<tr>\
							...
							<td>\
								<select data-placeholder="Selecione o tipo.."  id="novostatuspedido'+[i] +'" name="pedido_novostatus[]" tipodecomando class="select" tabindex="7">\
									<option value=""> - </option> \
							...
							<input type="hidden" id="pedidoaltera'+[i] +'" name="alterapedido[]" value="'+obj.pedido_numero[i] +'">\
							<input type="hidden" id="conta_pedido_antiga'+[i] +'" name="pedido_conta_corrente_antiga[]" value="'+obj.pedido_conta_corrente[i] +'">\
							<input type="hidden" id="valor_pedido_antiga'+[i] +'" name="pedido_valor_antiga[]" value="'+obj.pedido_valor[i] +'">\
							<input type="hidden" id="status_pedido_antiga'+[i] +'" name="pedido_status_antiga[]" value="'+obj.pedido_status[i] +'">\
						</tr>\
					');					
				}
				$('#form_pedidos').append('<input type="hidden" id="pedidosexistentes" name="pedidosexistentes" value="'+obj.pedido_caixa.length +'">'); 
		...
		var obj2 = JSON.parse(jsonStr);
			$(function(){
				...
				if ((obj2.status=='NA SUMEP')||(obj2.status=='SUMEP RESIDUO SIFBN')||(obj2.status=='SUMEP NAO LIQUIDADO')||(obj2.status=='SUMEP CONTA NAO DEBITADA')){
					$('#form_status_editar').append(
					'<select data-placeholder="Selecione o tipo.."  id="status_editado" name="status_editado" tipodecomando class="select form-control" tabindex="7">\
				...
				for (var i=0; i<obj2.historico_saldo.length; i++) {
					...
					$('#form_saldo_conta_2').append(
					'<tr>\
						<td><i class="icon-clock"> &nbsp Saldo Verificado em  '+ obj2.data_saldo[i] +'</i> &nbsp </td>\
						<td> <span class="label label-'+cordoefeito+'">'+ obj2.historico_saldo[i] +'</span> </td>\
						<td> &nbsp  <span class="pull-center badge bg-'+cordoefeito+' ">R$ '+ obj2.saldo_dispo[i] +'</span></td>\
						<td> &nbsp  <span class="pull-center badge bg-'+cordoefeito+' ">R$ '+ obj2.saldo_bloqueado[i] +'</span></td>\
						<td> &nbsp  <span class="pull-center badge bg-'+cordoefeito+' ">R$ '+ obj2.saldo_total[i] +'</span></td><br>\
					</tr><br>\
					'); 
				}
			});
			...
			$(function(){
				...
				for (var i=0; i<obj.contrato_caixa.length; i++) {
					...
					'<tr>\
						...
						<td>\
						  <select data-placeholder="Selecione o tipo.."  id="tipocarregado'+[i] +'" name="tipo[]" tipodecomando class="select" tabindex="2">\
								<option value=""> - </option> \
				...
				$('#form_contratos').append('<input type="hidden" id="contratosexistentes" name="vazio" value="'+obj.contrato_caixa.length +'">'); 
				$("[valordeamortizacao]").maskMoney({
					symbol:'', 
					decimal:',', 
					precision:2, 
